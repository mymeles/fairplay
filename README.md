...
- **Milestone 14 — Host Controls:** complete and deployed. New `apps/api/src/app/modules/host-control/`: `HostControlService` (pin/unpin — flips `host_pinned` and calls `ScoreRebuildService.recalculateEntry` so M09's `hostPinWeight=1000` immediately re-ranks; runner start/stop — delegates to `RunnerStateService.enable/disable` from M12, ownership verified via `SessionService.getSession`), `SessionSettingsService` (PATCH partial settings: shallow merge top-level keys, deep-merge `scoring` block, skip undefined to honor optional DTO fields, publish `session.updated`), `UpdateSessionSettingsDto` (every field optional with class-validator bounds matching `DEFAULT_SESSION_SETTINGS`, `forbidNonWhitelisted`), `HostQueueController` (pin/unpin at `/queue/:entryId/{pin,unpin}`), `HostRunnerController` (runner start/stop + settings PATCH at `/sessions/:id/...`). Extended `SpotifyPlaybackAdapter` with `skipToNext / pause / resume` (POST/PUT `/me/player/{next,pause,play}`, optional `device_id` query, 204→success, 404→`SPOTIFY_NO_ACTIVE_DEVICE`); extended `SpotifyDeviceService` + `HostDeviceController` with the three matching methods at `/host/spotify/{skip,pause,resume}`. Extended `QueueEntryRepository.setHostPinned` and `SessionRepository.updateSettings`. Veto (`POST /queue/:id/veto`) was already exposed by M10 — reused, not reimplemented. Shared types: `QueueUpdatedPayload.reason` now includes `host_pinned | host_unpinned`. No DB migration. 386 API tests pass (355 prior + 31 new); build/lint clean. Railway deploy `3c69c493-52be-4712-a19e-524963e2e0a5` succeeded; all 8 new endpoints (+ M10's veto) return 401 unauthenticated and the M12 runner + M13 poller continued ticking past the deploy. Hand-off: [03_milestones/HANDOFF_MILESTONE_14.md](03_milestones/HANDOFF_MILESTONE_14.md).
- **Milestone 15 — Session Token Economy:** complete and deployed. New `apps/api/src/app/modules/tokens/`: `TokenLedgerService` (durable grant/spend audit rows), `GuestWalletService` (guest wallet read + host grants), `BoostService` (guest boost spend → `queue_entries.boost_credits++` → score recalculation + Redis refresh), and controllers for `GET /guests/me/wallet`, `POST /queue/:entryId/apply-boost`, `POST /sessions/:sessionId/guests/:guestId/grant-tokens`. Added migration `20260516010000_m15_session_token_economy.sql` for `token_ledger` with positive grant / negative spend checks and a unique guard for per-entry spend idempotency. Join wallet creation now writes `JOIN_GRANT` ledger rows; challenge-lock now writes `CHALLENGE_LOCK` ledger rows. Shared realtime types now include `queue.updated{reason:boost_applied}` and `token.updated{reason:boost_applied|host_grant}`. No Spotify call is made by any token action. 407 API tests pass (386 prior + 21 new); build clean. Railway deploy `ef86ac8b-daf8-45f6-a3ea-b04ab856f741` succeeded. Hand-off: [03_milestones/HANDOFF_MILESTONE_15.md](03_milestones/HANDOFF_MILESTONE_15.md).
- **Milestone 16 — Moderation and Abuse Protection:** complete locally. New `apps/api/src/app/modules/moderation/`: `RateLimitService` (Redis token bucket for join/search/queue-add/vote/token-spend), `BlacklistService` (session track + artist blacklists), `GuestDisciplineService` (host mute/ban/unmute, removes pending/locked entries from Postgres + Redis), `ModerationService` (central guest status, rate-limit, explicit, and blacklist checks), and host controller endpoints. Added migration `20260516041149_m16_moderation_and_abuse_protection.sql` for `session_track_blacklist` and `session_artist_blacklist` with RLS deny-anon policies. Integrated moderation into join, search/normalize, queue add/list/remove, vote, boost, and challenge. Banned devices cannot rejoin when a device hash is present; muted/banned guests cannot mutate the visible queue. 428 API tests pass (407 prior + 21 new); build/lint clean with the existing `main.ts` console warning. Hand-off: [03_milestones/HANDOFF_MILESTONE_16.md](03_milestones/HANDOFF_MILESTONE_16.md).
...
- **API (Railway):** https://api-production-7ee5.up.railway.app/api/v1
  ...
  - `GET /sessions/:sessionId/queue` — guest-only. Returns queue entries ordered by score desc, createdAt asc, excluding REMOVED/VETOED. M16 blocks banned guests.
  - `DELETE /queue/:entryId` — guest-only. The adder may retract their own PENDING entry. M16 blocks muted/banned guests from mutating the queue.
  ...
  - `GET /guests/me/wallet` — guest-only (M15). Returns the caller's free token wallet for the JWT-scoped session.
  - `POST /queue/:entryId/apply-boost` — guest-only (M15). Consumes one boost token once per guest/entry, increments `boostCredits`, recalculates score, and publishes `queue.updated{reason:boost_applied}` + `token.updated{reason:boost_applied}`. No Spotify calls.
  - `POST /queue/:entryId/challenge-lock` — guest-only (M10/M15). Consumes one free challenge token from the caller's session wallet, records a token ledger row, and unlocks a `LOCKED` entry back to `PENDING`; does not call Spotify and does not guarantee playback.
  ...
  - `POST /queue/:entryId/pin` / `unpin` — host-only (M14). Flips `host_pinned` and re-ranks via M09's scoring (pinned entries jump to the top via `hostPinWeight=1000`). Publishes `queue.updated{reason:host_pinned|host_unpinned}`.
  - `POST /sessions/:sessionId/guests/:guestId/grant-tokens` — host-only (M15). Body `{ "boostTokens"?: number, "challengeTokens"?: number }`; grants free session tokens, records ledger rows, and publishes `token.updated{reason:host_grant}`.
  - `POST /sessions/:sessionId/blacklist/track` — host-only (M16). Body `{ "spotifyTrackId"?: string, "spotifyUri"?: "spotify:track:...", "title"?: string }`; rejects future queue adds for that track and filters it from search results.
  - `POST /sessions/:sessionId/blacklist/artist` — host-only (M16). Body `{ "artistName": "..." }`; rejects future queue adds for matching artist names and filters them from search results.
  - `POST /sessions/:sessionId/guests/:guestId/mute` — host-only (M16). Sets guest status to `MUTED`, removes their pending/locked entries from the visible queue, and blocks future queue-affecting actions.
  - `POST /sessions/:sessionId/guests/:guestId/ban` — host-only (M16). Sets guest status to `BANNED`, removes pending/locked entries, blocks guest actions, and blocks rejoin by the same `deviceHash`.
  - `DELETE /sessions/:sessionId/guests/:guestId/mute` — host-only (M16). Restores a muted guest to `ACTIVE`; banned guests cannot be unmuted through this endpoint.
  - `POST /sessions/:sessionId/runner/start` / `stop` — host-only (M14). Toggles `RunnerStateService` per-session enabled flag; publishes `runner.status_changed`.
  - `PATCH /sessions/:sessionId/settings` — host-only (M14). Partial settings update (any subset of session settings + nested `scoring` weights). Publishes `session.updated`.
  - `POST /host/spotify/skip` / `pause` / `resume` — host-only (M14). Thin pass-throughs to Spotify `POST /me/player/next` / `PUT /me/player/{pause,play}` using the host's selected device.
